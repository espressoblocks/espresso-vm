...
const defaultBlockPackages = {
    ...
    espresso_strings: require('../blocks/espresso_strings.js'),
...
/**
 * espresso: String methods. dunno if i should be doing this in the runtime but oh well :BLEH:
 * @type {object}
 */
const strings = {
    /**
     ...
     */
    random:() => {
        //get random amount of characters
        let max = 10;
        let min = 1;
    ...
    /**
     * Encode a string with various methods
     * @param {string} txt 
     * @param {string} method 
     */
    encode: (txt,method)=>{
        if (method == "BASE64") {
            return Buffer.from(txt).toString('base64'); //since btoa is deprecated
        } else if (method == "BASE32") {
            return base32.encode(txt);
        } else if (method == "UUENCODE") {
            return uuencode.encode(txt);
        } else if (method == "NUMBER") {
            //credit to sharkpool
            return txt.split("").map(char => char.charCodeAt(0)).map(value => value.toString().padStart(3, "0")).join("");
        } else if (method == "URL") {
    ...
    decode: (txt,method)=>{
        try {
            if (method == "BASE64") {
                return Buffer.from(txt,'base64').toString('utf-8'); //since atob is deprecated
            } else if (method == "BASE32") {
                return base32.decode(txt);
            } else if (method == "UUENCODE") {
                return uuencode.decode(txt);
            } else if (method == "NUMBER") {
                //credit to sharkpool
                const decodedString = [];
...
class Runtime extends EventEmitter {
    constructor () {
        ...
        /**
         * espresso: The strings things idk fdsgmofhmgiuhngui
         */
