...
class JSGenerator {
    ...
    descendInput (node, visualReport = false) {
        ...
        case 'sensing.unixtimestamp':
            return new TypedInput(`(new Date().getTime())`, TYPE_NUMBER);
        case 'sensing.getdatalocal':
            let argument = node.left.value;
            //                                                                          im encoding it so you cant escape the string and execute js
            return new TypedInput(`(localStorage.getItem('Espresso-LocalStorageBlocks-' + atob("${btoa(argument)}")) ?? "")`, TYPE_STRING);
        /*
        case 'sensing.getdata':
            let string = node.left.value;
            return new TypedInput(`runtime.ext_scratch3_sensing.getData({'DATA_KEY':'${string}'})`, TYPE_STRING);*/
        case 'timer.get':
        ...
        case 'strings.reverse':
            let arg = node.left.value;
            return new TypedInput(`vm.runtime.strings.reverse("${arg}")`, TYPE_STRING)
