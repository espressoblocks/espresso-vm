...
class Scratch3SensingBlocks {
    ...
    getPrimitives () {
        return {
            ...
            sensing_uploaddata: this.uploadData,
            sensing_getdata: this.getData,
            sensing_unixtimestamp: this.unixTimestamp,
            sensing_uploaddatalocal: this.setDataLocal,
            sensing_getdatalocal: this.getDataLocal
        };
    }
    ...
    async uploadData(args) {
        let request = await fetch(`https://espressoblocks-dumobase.replit.app/api/${encodeURIComponent(args.DATA_KEY)}`, {
            method: 'POST',
            headers: {
                'Content-Type': 'application/json'
            },
            body: JSON.stringify({
                value: args.DATA_VALUE
            })
        }).then(data => data.json());
        console.log(
          "%cEspresso",
          "color: white; font-weight: 600; background: black; padding: 0 5px; border-radius: 5px",
          request
        );
    }

    async getData(args) {
        let request = await fetch(`https://espressoblocks-dumobase.replit.app/api/${encodeURIComponent(args.DATA_KEY)}`).then(data => data.json());
        if ('value' in request) {
    ...
    setDataLocal(args) {
        localStorage.setItem(`Espresso-LocalStorageBlocks-${args.DATA_KEY}`, args.DATA_VALUE);
    }

    getDataLocal(args) {
        return localStorage.getItem(`Espresso-LocalStorageBlocks-${args.DATA_KEY}`);
    }
}
